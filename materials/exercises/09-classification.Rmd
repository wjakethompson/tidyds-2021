...
set.seed(100) # Important!
tree_fit <- fit(________, 
                ________, 
                ________)

tree_fit

predict(________, new_data = ________)
```
...
```{r}
tree_predict <- ________(tree_fit, new_data = ________)
...
all_preds <- ________ %>%
  select(remote) %>%
  bind_cols(predict(________, new_data = ________)) %>%
  bind_cols(predict(________, new_data = ________, type = ________))

________(all_preds, ________, ________)
```
...
roc_curve(all_preds, ________, ________) %>%
  ________
```
